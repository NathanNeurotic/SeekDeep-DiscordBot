...
backup = Path(f"index.js.bak-status-metrics-{stamp}")
...
if "SEEKDEEP_STATUS_METRICS_START" in text:
    start = text.find("// SEEKDEEP_STATUS_METRICS_START")
    end = text.find("// SEEKDEEP_STATUS_METRICS_END", start)

    if end == -1:
        raise SystemExit("Found status metrics start marker but no end marker.")

    end += len("// SEEKDEEP_STATUS_METRICS_END")
    text = text[:start] + metrics_helpers.strip() + "\n\n" + text[end:].lstrip()
    print("[SeekDeep] Replaced status metrics helpers.")
else:
    anchor = "// SEEKDEEP_RESPONSE_FOOTER_START"
    pos = text.find(anchor)

    if pos == -1:
        anchor = "async function statusText()"
        pos = text.find(anchor)

    if pos == -1:
        raise SystemExit("Could not find insertion point for status metrics helpers.")

    text = text[:pos] + metrics_helpers.strip() + "\n\n" + text[pos:]
...
if "function seekdeepAppendResponseFooter(" in text:
    ...
    raise SystemExit("Could not find seekdeepAppendResponseFooter. Apply the response footer patch first.")
...
print("[SeekDeep] Status metrics patch written.")
