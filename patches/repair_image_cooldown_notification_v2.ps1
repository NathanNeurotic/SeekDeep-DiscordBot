# SeekDeep / Seekotics image cooldown notification repair v2
#
# Fixes:
# - Cooldown works, but repeated image requests silently do nothing.
#
# Strategy:
# - Do NOT locate the existing internal cooldown rejection branch.
# - Instead, patch the message image route before it reaches the silent gate:
#     if (isNaturalImagePrompt(prompt)) {
#       const cooldownRemaining = seekdeepImageCooldownRemaining(message.author.id);
#       if (cooldownRemaining > 0) reply with seconds left and return;
#       ...
#     }
#
# This preserves the existing cooldown gate. It only adds the missing visible response.
#
# Required checks:
#   node --check .\index.js
...
try {
  ...
  $backupPath = Join-Path $backupDir "index.js.cooldown-notification-repair-v2-$stamp.bak"
...
'@
  ...
  $patchPyPath = Join-Path $patchesDir "repair_image_cooldown_notification_v2.py"
  ...
  try {
    Write-SeekDeepInfo "Applying image cooldown notification repair v2"
    & ".\.venv\Scripts\python.exe" $patchPyPath $indexPath
    if ($LASTEXITCODE -ne 0) {
      throw "Python repair helper failed with exit code $LASTEXITCODE."
    }
    Write-SeekDeepPass "Applied image cooldown notification repair v2"
  ...
  Write-Host ""
  Write-Host "SeekDeep image cooldown notification repair v2 completed successfully." -ForegroundColor Green
