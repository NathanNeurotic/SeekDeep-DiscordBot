# SeekDeep / Seekotics Discord-thread archive storage patch
#
# Goal:
# - Move new image archives away from local disk as the primary archive backend.
# - Use one archive channel per server.
# - Use one archive thread per user inside that server archive channel.
# - Keep local disk as a last-resort fallback only if Discord archive posting fails.
#
# Adds:
# - seekdeepGetOrCreateGuildArchiveChannel(...)
# - seekdeepGetOrCreateUserArchiveThread(...)
# - seekdeepArchiveImageStateToDiscordThread(...)
# - seekdeepMigrateLocalArchiveToThreads(...)
# - @SEEKOTICS migratearchive / @SEEKOTICS archive migrate message command
#
# Default archive channel:
# - Channel ID from .env:
#     SEEKDEEP_ARCHIVE_CHANNEL_ID=<channel id>
# - Otherwise channel name from .env:
#     SEEKDEEP_ARCHIVE_CHANNEL_NAME=seekdeep-archive
# - Otherwise default name:
#     seekdeep-archive
#
# Important:
# - This patch changes NEW Archive button presses to post into Discord threads.
# - It does not delete old local archives.
# - Local fallback remains available if Discord permissions fail.
#
...
# - Manage Threads is helpful for unarchiving old archive threads.
#
# Files patched:
# - index.js only
...
function Write-Info {
  ...
  Write-Host "[SeekDeep thread-archive] $Message" -ForegroundColor Cyan
}
...
try {
  ...
  $backup = Join-Path $backupDir "index.js.thread-archive-storage-$stamp.bak"
...
'@
  ...
  $patchPyPath = Join-Path $patchesDir "patch_thread_archive_storage.py"
  [System.IO.File]::WriteAllText($patchPyPath, $patchPy, [System.Text.UTF8Encoding]::new($false))

  Push-Location $projectRoot
  try {
    Write-Info "Applying Discord-thread archive storage patch"
    & ".\.venv\Scripts\python.exe" $patchPyPath $indexPath
    if ($LASTEXITCODE -ne 0) { throw "Patch helper failed with exit code $LASTEXITCODE." }
    Write-Pass "Applied Discord-thread archive storage patch"
  ...
  Write-Host ""
  Write-Pass "Discord-thread archive storage patch completed."
  Write-Host "Backup created: $backup" -ForegroundColor Yellow
  Write-Host "Restart the bot before testing." -ForegroundColor Yellow
  Write-Host ""
  Write-Host "Optional .env settings:" -ForegroundColor Cyan
  Write-Host "SEEKDEEP_ARCHIVE_CHANNEL_ID=<existing channel id>" -ForegroundColor White
  Write-Host "SEEKDEEP_ARCHIVE_CHANNEL_NAME=seekdeep-archive" -ForegroundColor White
  Write-Host ""
  ...
  Write-Host "   Expected: archive channel/thread is created and image is posted there." -ForegroundColor White
  Write-Host "3) Message command: @SEEKOTICS migratearchive" -ForegroundColor White
  Write-Host "   Expected: imports up to 25 old local images into your user archive thread." -ForegroundColor White
  exit 0
} catch {
  Write-Host ""
