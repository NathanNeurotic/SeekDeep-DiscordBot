# SeekDeep / Seekotics image intent routing v2
#
# Fixes observed misses from v1:
# - "Pepe and Sailor Moon smoking on a balcony at sunset" routed to chat
# - "make a metal album cover with a burning cathedral" routed to chat
# - "can I see a Siamese kitten in armor" routed to chat
#
# Cause:
# - The text-question guard was too aggressive:
#   "can I..." and "make..." style visual requests were being treated as chat too early.
#
# Files patched:
# - index.js
#
...
function Write-Info {
  ...
  Write-Host "[SeekDeep intent-router-v2] $Message" -ForegroundColor Cyan
}
...
try {
  ...
  $indexBackup = Join-Path $backupDir "index.js.image-intent-routing-v2-$stamp.bak"
...
'@
  ...
  $patchPyPath = Join-Path $patchesDir "patch_image_intent_routing_v2.py"
  [System.IO.File]::WriteAllText($patchPyPath, $patchPy, $utf8NoBom)

  Push-Location $projectRoot
  try {
    Write-Info "Applying image intent routing v2 patch"
    & ".\.venv\Scripts\python.exe" $patchPyPath $indexPath
    if ($LASTEXITCODE -ne 0) {
      throw "Patch helper failed with exit code $LASTEXITCODE."
    }
    Write-Pass "Applied image intent routing v2 patch"
  ...
  Write-Host ""
  Write-Pass "Image intent routing v2 patch completed."
