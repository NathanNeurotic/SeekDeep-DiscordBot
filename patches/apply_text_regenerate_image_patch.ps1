# SeekDeep / Seekotics text regenerate patch
# Adds @SEEKOTICS regenerate / regen / reroll / redo as a hard local image command.
# Preserves:
# - stabilized dispatcher order
# - post archive hard-command routing
# - seekdeepEnqueueImageJob(job, runner)
# - existing Regenerate button behavior
#
...
try {
  ...
  $backupPath = Join-Path $backupDir "index.js.text-regenerate-$stamp.bak"
...
'@
  ...
  $patchPyPath = Join-Path $patchesDir "patch_text_regenerate_image.py"
  ...
  try {
    Write-SeekDeepInfo "Applying text regenerate patch"
    & ".\.venv\Scripts\python.exe" $patchPyPath $indexPath
    if ($LASTEXITCODE -ne 0) {
      throw "Python patch helper failed with exit code $LASTEXITCODE."
    }
    Write-SeekDeepPass "Applied text regenerate patch"
  ...
  Write-Host ""
  Write-Host "SeekDeep text regenerate patch completed successfully." -ForegroundColor Green
  Write-Host "Backup created: $backupPath" -ForegroundColor Green
  Write-Host "Test in Discord: @SEEKOTICS regenerate" -ForegroundColor Yellow
  exit 0
} catch {
  Write-Host ""
