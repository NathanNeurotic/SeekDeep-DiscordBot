...
$IndexBackupPath = Join-Path $BackupsDir "index.js.before-shared-archive-button-ack-v4-$Stamp.bak"
$PatchJsPath = Join-Path $PatchesDir "apply_shared_archive_button_ack_v4_$Stamp.cjs"
...
try {
  ...
  Write-Host "- Shared Archive button handler is inserted before the interactionCreate listener instead of inside the listener argument list."
  Write-Host "- Shared Archive button clicks are acknowledged immediately before archive work begins."
  ...
  Write-Host "- Failures now return an ephemeral reason and log '[SeekDeep] shared archive button failed'."
} catch {
  Write-Host ""
