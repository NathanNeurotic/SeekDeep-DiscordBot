# SeekDeep / Seekotics quiet generation report v2 + auto-recovery
#
# This fixes the failed v1 quiet-report patch.
#
# What it does:
# 1. Automatically restores the newest quiet-generation-report backup if one exists.
# 2. Applies a safer quiet-report patch that only touches:
#    - seekdeepAppendResponseFooter(...)
#    - display helper functions
#    - simple public-facing strings
# 3. Runs:
#    - node --check .\index.js
#    - .\.venv\Scripts\python.exe -m py_compile .\local_ai_server.py
#
# This avoids the v1 mistake that left a broken syntax fragment around line 159.
...
function Write-Info {
  ...
  Write-Host "[SeekDeep quiet-report-v2] $Message" -ForegroundColor Cyan
}
...
try {
  ...
  $preRepairBackup = Join-Path $backupDir "index.js.before-quiet-report-v2-recover-$stamp.bak"
  Copy-Item -LiteralPath $indexPath -Destination $preRepairBackup -Force
  Write-Pass "Backed up current index.js to $preRepairBackup"

  # Auto-restore the last known pre-v1 state if the failed v1 backup exists.
  $quietBackup = Get-ChildItem -LiteralPath $backupDir -Filter "index.js.quiet-generation-report-*.bak" -File |
    Sort-Object LastWriteTime -Descending |
    Select-Object -First 1

  if ($quietBackup) {
    Copy-Item -LiteralPath $quietBackup.FullName -Destination $indexPath -Force
    Write-Pass "Restored failed v1 backup first: $($quietBackup.FullName)"
  } else {
    Write-Warn "No quiet-generation-report backup found. Applying v2 to current index.js."
  }
...
'@
  ...
  Write-Host ""
  Write-Pass "Quiet report v2 completed."
  Write-Host "Current broken file backup: $preRepairBackup" -ForegroundColor Yellow
  if ($quietBackup) { Write-Host "Restored-from backup: $($quietBackup.FullName)" -ForegroundColor Yellow }
  ...
  if ($preRepairBackup) { Write-Host $preRepairBackup -ForegroundColor Yellow }
  if ($quietBackup) { Write-Host $quietBackup.FullName -ForegroundColor Yellow }
  exit 1
}
