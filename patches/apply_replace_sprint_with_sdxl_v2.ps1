# SeekDeep / Seekotics image model replacement v2: Sana Sprint -> SDXL Base
#
...
try {
  ...
  $indexBackup = Join-Path $backupDir "index.js.replace-sprint-with-sdxl-v2-$stamp.bak"
  $serverBackup = Join-Path $backupDir "local_ai_server.py.replace-sprint-with-sdxl-v2-$stamp.bak"
  $envBackup = Join-Path $backupDir ".env.replace-sprint-with-sdxl-v2-$stamp.bak"
...
if len(sys.argv) != 4:
    raise SystemExit("Usage: patch_replace_sprint_with_sdxl_v2.py <index.js> <local_ai_server.py> <.env>")
...
'@
  ...
  $patchPyPath = Join-Path $patchesDir 'patch_replace_sprint_with_sdxl_v2.py'
