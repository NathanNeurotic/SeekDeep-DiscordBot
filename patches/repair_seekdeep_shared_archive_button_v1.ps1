...
$IndexBackupPath = Join-Path $BackupsDir "index.js.before-shared-archive-button-v1-$Stamp.bak"
$PatchJsPath = Join-Path $PatchesDir "apply_shared_archive_button_v1_$Stamp.cjs"
...
try {
  ...
  Write-Host "- Generated images now show Archive and Shared Archive buttons."
  Write-Host "- Shared Archive posts to a server-wide shared archive thread."
  Write-Host "- Shared thread name uses: coin bullet Shared Archive bullet tracked count."
  Write-Host "- Shared Archive uses the same per-server archive channel setup and permission checks."
  Write-Host "- Help now lists Shared Archive in image/archive sections."
} catch {
  Write-Host ""
