# SeekDeep / Seekotics image button router + ack hard-fix
#
# Fix target:
# - Clicking Original / Refined / Both shows "This interaction failed"
# - No queueing occurs after button click
# - Console only shows the initial route=image log
#
# Strategy:
# 1) Repair seekdeepHandleImageButton so it has exactly one safe customId/match prelude
#    and immediately deferUpdate() for supported button ids.
# 2) Hard-wire interactionCreate to route supported seekdeep button customIds directly
#    into seekdeepHandleImageButton before other branches can swallow them.
#
# Safe patching behavior:
# - Makes a backup first
# - Uses anchored text transforms only
...
function Write-Info { param([string]$Message) Write-Host "[SeekDeep button-router-fix] $Message" -ForegroundColor Cyan }
...
try {
  ...
  $backup = Join-Path $backupDir "index.js.button-router-fix-$stamp.bak"
...
'@
  ...
  Write-Host ""
  Write-Pass "Button router hard-fix completed."
  ...
  Write-Host "- no 'This interaction failed'" -ForegroundColor White
  Write-Host "- queue begins immediately after click" -ForegroundColor White
  Write-Host "- bot stops typing after it responds" -ForegroundColor White
  exit 0
}
catch {
  Write-Host ""
