# SeekDeep / Seekotics reply-context image prompt patch
#
# Goal:
# - If a user replies to a message with @SEEKOTICS and/or "generate",
#   the bot should use the replied-to message content as the image prompt context.
# - This fixes flows like:
#     @SEEKOTICS generate   (while replying to "Predator spyro")
#   so the bot generates Predator spyro instead of asking for a prompt.
#
# Safety:
...
function Write-Info {
  ...
  Write-Host "[SeekDeep reply-context] $Message" -ForegroundColor Cyan
}
...
try {
  ...
  $indexBackup = Join-Path $backupDir "index.js.reply-context-image-prompt-$stamp.bak"
...
'@
  ...
  $patchPyPath = Join-Path $patchesDir "patch_reply_context_image_prompt.py"
  [System.IO.File]::WriteAllText($patchPyPath, $patchPy, $utf8NoBom)

  Push-Location $projectRoot
  try {
    Write-Info "Applying reply-context image prompt patch"
    & ".\.venv\Scripts\python.exe" $patchPyPath $indexPath
    if ($LASTEXITCODE -ne 0) {
      throw "Patch helper failed with exit code $LASTEXITCODE."
    }
    Write-Pass "Applied reply-context image prompt patch"
  ...
  Write-Host ""
  Write-Pass "Reply-context image prompt patch completed."
  ...
  Write-Host ""
  Write-Host "Retest these:" -ForegroundColor Cyan
  Write-Host "1) Reply to: Predator spyro" -ForegroundColor White
  Write-Host "   Then send: @SEEKOTICS generate" -ForegroundColor White
  Write-Host "   Expected: route=image, prompt=Predator spyro" -ForegroundColor White
  Write-Host "" 
  Write-Host "2) Reply to: Toad from Mario" -ForegroundColor White
  Write-Host "   Then send: @SEEKOTICS" -ForegroundColor White
  Write-Host "   Expected: route=image or image-followup using replied text" -ForegroundColor White
  Write-Host ""
  Write-Host "3) Direct non-reply prompt still works:" -ForegroundColor White
