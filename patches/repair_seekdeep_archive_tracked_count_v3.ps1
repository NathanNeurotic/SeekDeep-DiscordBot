...
$BackupPath = Join-Path $BackupsDir "index.js.before-archive-tracked-count-v3-$Stamp.bak"
Copy-Item $IndexPath $BackupPath -Force
Write-Host "Backup created: $BackupPath"

$PatchJsPath = Join-Path $PatchesDir "apply_archive_tracked_count_v3_$Stamp.cjs"
@'
const fs = require('fs');
...
try {
  ...
  Write-Host "Archive thread counts now use tracked archived-image posts only, never Discord message/thread counts."
  Write-Host "Repair command: @SeekDeep archive count set 1"
} catch {
