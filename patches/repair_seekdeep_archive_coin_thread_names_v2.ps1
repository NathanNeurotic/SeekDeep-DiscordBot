...
$BackupPath = Join-Path $BackupsDir "index.js.before-archive-coin-thread-names-v2-$Stamp.bak"
Copy-Item $IndexPath $BackupPath -Force
Write-Host "Backup created: $BackupPath"

$PatchJsPath = Join-Path $PatchesDir "apply_archive_coin_thread_names_v2_$Stamp.cjs"
@'
const fs = require('fs');
...
const helperBlock = [
  ...
  '  await message.reply({',
  "    content: ['Archive thread titles are automatic now:', '`\\u{1FA99} \\u2022 Archive \\u2022 current nickname \\u2022 current amount`', 'The nickname and amount update when archives are posted.'].join('\\n'),",
...
const newGetOrCreateUserArchiveThread = [
  ...
  "      'Thread format: coin emoji / bullet / Archive / current nickname / current amount',",
...
console.log('Patched archive coin/bullet thread naming encoding-safe v2 successfully.');
...
try {
  ...
  Write-Host "Thread format: coin emoji / bullet / Archive / current nickname / current amount"
} catch {
