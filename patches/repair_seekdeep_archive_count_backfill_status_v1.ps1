...
$IndexBackupPath = Join-Path $BackupsDir "index.js.before-archive-count-backfill-status-v1-$Stamp.bak"
$PatchJsPath = Join-Path $PatchesDir "apply_archive_count_backfill_status_v1_$Stamp.cjs"
...
try {
  ...
  Write-Host "- Existing archive entries are counted by scanning SeekDeep Image Archive Entry messages, not Discord thread message totals."
  Write-Host "- Opening an existing archive backfills the tracked count before renaming the thread."
  Write-Host "- Archive status finds user threads by stored thread ID, legacy name, or fuzzy nickname match instead of only count-0 names."
  Write-Host "- Archive status can target mentioned users, e.g. @SeekDeep archive status @user."
} catch {
  Write-Host ""
