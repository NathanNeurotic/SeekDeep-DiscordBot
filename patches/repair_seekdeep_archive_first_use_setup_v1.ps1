...
$BackupPath = Join-Path $BackupsDir "index.js.before-archive-first-use-setup-v1-$Stamp.bak"
Copy-Item $IndexPath $BackupPath -Force
Write-Host "Backup created: $BackupPath"

$PatchJsPath = Join-Path $PatchesDir "apply_archive_first_use_setup_v1_$Stamp.cjs"
@'
const fs = require('fs');
...
function findFunctionRange(src, functionName) {
  ...
  const signature = new RegExp('(?:async\\s+)?function\\s+' + escaped + '\\s*\\(');
...
const helperBlock = [
  ...
  '      required,',
  "      '',",
  "      'Nothing was archived to a master/local fallback. Setup must be completed first.'",
  "    ].join('\\n');",
  '  }',
  '  return [',
  "    'Archive storage is not configured for this server yet.',",
  ...
  '      missingLines,',
  "      '',",
  "      'Nothing was archived to a master/local fallback.'",
  "    ].join('\\n');",
  '  }',
  '  return [',
  "    'Archive channel is configured, but SeekDeep is missing permissions in ' + channelText + '.',",
  ...
  '',
  '  await message.reply({',
  "    content: ['Archive channel assigned for this server: <#' + requestedChannel.id + '>', 'Future archives will use this server-assigned channel only.'].join('\\n'),",
  '    allowedMentions: { repliedUser: false },',
  '  });',
  '  return true;',
  '}',
...
source = source.replace(
  ...
  "content: (err?.code === 'SEEKDEEP_ARCHIVE_NOT_CONFIGURED' || err?.code === 'SEEKDEEP_ARCHIVE_PERMISSIONS_MISSING') ? err.message : 'Archive lookup failed locally. Check the bot console for details.',"
);

source = source.replace(
  "      console.warn('Discord thread archive failed; falling back to local archive:', err?.message || err);",
  gateInsert + "      console.warn('Discord thread archive failed; falling back to local archive:', err?.message || err);"
);
source = source.replace(
  "      console.warn('Emergency Discord thread archive failed; falling back to local archive:', err?.message || err);",
  gateInsert + "      console.warn('Emergency Discord thread archive failed; falling back to local archive:', err?.message || err);"
);

if (source === before) throw new Error('Patch made no changes; refusing to continue.');

fs.writeFileSync(indexPath, source, 'utf8');
console.log('Patched first-use archive setup flow successfully.');
...
try {
  ...
  Write-Host "Archive config path: .\data\archive-guild-config.json"
} catch {
