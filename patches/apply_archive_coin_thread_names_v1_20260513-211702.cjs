...
const helperBlock = [
  ...
  "    .replace(/\\s+/g, ' ')",
  '    .trim();',
  "  return Array.from(clean || 'ðŸª™ â€¢ Archive â€¢ unknown â€¢ 0').slice(0, 96).join('').trim() || 'ðŸª™ â€¢ Archive â€¢ unknown â€¢ 0';",
  '}',
  '',
  ...
  "  return seekdeepArchiveThreadClampName(String(process.env.SEEKDEEP_ARCHIVE_THREAD_EMOJI || 'ðŸª™')).slice(0, 8).trim() || 'ðŸª™';",
  '}',
  '',
  'function seekdeepArchiveThreadBullet() {',
  "  return seekdeepArchiveThreadClampName(String(process.env.SEEKDEEP_ARCHIVE_THREAD_BULLET || 'â€¢')).slice(0, 4).trim() || 'â€¢';",
  '}',
  '',
  ...
  '  await message.reply({',
  "    content: ['Archive thread titles are automatic now:', '`ðŸª™ â€¢ Archive â€¢ current nickname â€¢ current amount`', 'The nickname and amount update when archives are posted.'].join('\\n'),",
...
const newArchiveUserThreadName = [
  ...
  "    .replace(/-+/g, '-')",
  "    .slice(0, 48) || 'unknown-user';",
  "  return ('ðŸª™ â€¢ Archive â€¢ ' + username + ' â€¢ ' + (Math.max(0, Number(count || 0) || 0))).slice(0, 90);",
  '}'
].join('\n');
...
const newGetOrCreateUserArchiveThread = [
  ...
  '    });',
  '    await thread.send([',
  "      'ðŸª™ SeekDeep archive for ' + (user?.id ? '<@' + user.id + '>' : 'unknown user') + '.',",
  "      'Thread format: ðŸª™ â€¢ Archive â€¢ current nickname â€¢ current amount',",
