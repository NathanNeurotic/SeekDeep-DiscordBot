# SeekDeep / Seekotics guild-scoped archive isolation patch
#
# Fixes:
# - Archive state/thread mapping can be shared across servers.
# - Archive commands should be guild-local by default.
#
# Behavior after patch:
# - Every archive key is namespaced by guild ID.
# - Same user in two servers gets separate archive/thread state.
# - DM archive state is separated as dm:<userId> if archive is used in DMs.
# - Existing old global archive keys are not deleted; new writes use scoped keys.
#
# This is a defensive compatibility patch:
# - It does not assume one exact archive implementation.
# - It adds a central archive-scope helper.
# - It rewrites common archive key/map/file patterns to include guild scope.
# - It wraps archive path helpers when present.
#
# Safety:
...
function Write-Info {
  ...
  Write-Host "[SeekDeep guild-archive-scope] $Message" -ForegroundColor Cyan
}
...
try {
  ...
  $indexBackup = Join-Path $backupDir "index.js.guild-archive-scope-$stamp.bak"
...
'@
  ...
  $patchPyPath = Join-Path $patchesDir "patch_guild_archive_scope.py"
  [System.IO.File]::WriteAllText($patchPyPath, $patchPy, $utf8NoBom)

  Push-Location $projectRoot
  try {
    Write-Info "Applying guild-scoped archive patch"
    & ".\.venv\Scripts\python.exe" $patchPyPath $indexPath
    if ($LASTEXITCODE -ne 0) {
      throw "Patch helper failed with exit code $LASTEXITCODE."
    }
    Write-Pass "Applied guild-scoped archive patch"
  ...
  Write-Host ""
  Write-Pass "Guild-scoped archive patch completed."
  ...
  Write-Host "1) Archive an image in Server A." -ForegroundColor White
  Write-Host "2) Archive an image from the same user in Server B." -ForegroundColor White
  Write-Host "3) Confirm the archive/thread/state no longer crosses between servers." -ForegroundColor White
  Write-Host ""
  Write-Host "Note: old global archive records are not deleted or migrated automatically." -ForegroundColor Yellow
  exit 0
} catch {
  Write-Host ""
