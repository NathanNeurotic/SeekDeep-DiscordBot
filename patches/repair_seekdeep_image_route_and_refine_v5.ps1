...
if (-not (Test-Path $IndexPath)) { throw "index.js not found at $IndexPath. Run this from the SeekDeep-DiscordBot project root." }
if (-not (Test-Path $LocalAiPath)) { throw "local_ai_server.py not found at $LocalAiPath. Run this from the SeekDeep-DiscordBot project root." }

New-Item -ItemType Directory -Path $BackupsDir -Force | Out-Null
New-Item -ItemType Directory -Path $PatchesDir -Force | Out-Null

$Stamp = Get-Date -Format 'yyyyMMdd-HHmmss'
$IndexBackupPath = Join-Path $BackupsDir "index.js.before-image-route-refine-v5-$Stamp.bak"
$LocalAiBackupPath = Join-Path $BackupsDir "local_ai_server.py.before-image-route-refine-v5-$Stamp.bak"
$PatchJsPath = Join-Path $PatchesDir "apply_image_route_refine_v5_$Stamp.cjs"
...
const localAiPath = path.join(process.cwd(), 'local_ai_server.py');
let indexSource = fs.readFileSync(indexPath, 'utf8').replace(/\r\n/g, '\n');
let pySource = fs.readFileSync(localAiPath, 'utf8').replace(/\r\n/g, '\n');
const changes = [];
...
function replaceBetween(src, startMarker, endMarker, replacement, label) {
  ...
  if (start < 0 || end < 0) fail('Could not find marker block for ' + label);
  changes.push(label);
  return src.slice(0, start) + replacement + src.slice(end + endMarker.length);
}

function replaceOnce(src, find, replacement, label, required = true) {
  ...
  if (idx < 0) {
    if (required) fail('Could not find anchor for ' + label);
...
function insertBefore(src, marker, block, label) {
  ...
  if (idx < 0) fail('Could not find insertion marker for ' + label);
  changes.push(label);
  return src.slice(0, idx) + block + '\n\n' + src.slice(idx);
}

const promptRefineBlock = [
...
"const SEEKDEEP_IMAGE_PROMPT_MAX_CHARS = Math.max(180, Number(process.env.SEEKDEEP_IMAGE_PROMPT_MAX_CHARS || 300));",
'',
...
indexSource = replaceBetween(indexSource, '// SEEKDEEP_IMAGE_PROMPT_REFINEMENT_START', '// SEEKDEEP_IMAGE_PROMPT_REFINEMENT_END', promptRefineBlock, 'replaced prompt refinement block');

const rawPromptInfoOld = [
'    promptInfo = {',
...
indexSource = replaceOnce(indexSource, rawPromptInfoOld, rawPromptInfoNew, 'added negativePrompt to raw image promptInfo', false);

const imagePayloadOld = [
...
indexSource = replaceOnce(indexSource, imagePayloadOld, imagePayloadNew, 'added negative_prompt to /image payload', true);
...
indexSource = insertBefore(indexSource, 'function isNaturalStatusPrompt(prompt) {', directHelpers, 'inserted direct image alias helpers');
...
"      await sendLongMessageReply(message, ['No pending confirmation command is active.', '', 'Use a full command instead:', '@SEEKOTICS draw me <image idea>', '@SEEKOTICS generate <image idea>'].join('\\n'));",
'      return;',
'    }',
'',
...
indexSource = replaceOnce(indexSource, '    const utilityKind = seekdeepUtilityPromptKind(prompt);', directRoute, 'inserted direct image alias route before suggestions', true);
...
pySource = replaceOnce(pySource, pyClassOld, pyClassNew, 'added negative_prompt to ImageRequest', true);
...
pySource = replaceOnce(pySource, pyArgsOld, pyArgsNew, 'added request_negative_prompt to image args', true);
...
pySource = replaceOnce(pySource, pyEnvNegOld, pyEnvNegNew, 'made env negative prompt fallback non-overriding', false);
...
try {
  ...
  Write-Host "- Valid generate/draw image aliases route before suggestion guards."
  Write-Host "- Bare Yes/Okay replies are handled locally instead of going to Qwen."
  Write-Host "- Refined prompts no longer put avoid/no/malformed/plastic terms in the positive prompt."
  Write-Host "- negative_prompt is sent separately to local_ai_server.py."
} catch {
  Write-Host ""
