# SeekDeep / Seekotics regenerate cooldown repair
#
# Fixes:
# - Regenerate bypasses the 45-second image cooldown.
#
# Adds cooldown enforcement to:
# - text regenerate route, if present
# - Regenerate button handler, if present
#
# Preserves:
# - working image generation
# - working refined prompt display
# - working cooldown notification sender
...
try {
  ...
  $backupPath = Join-Path $backupDir "index.js.regenerate-cooldown-repair-$stamp.bak"
...
'@
  ...
  $patchPyPath = Join-Path $patchesDir "repair_regenerate_cooldown.py"
  ...
  try {
    Write-SeekDeepInfo "Applying regenerate cooldown repair"
    & ".\.venv\Scripts\python.exe" $patchPyPath $indexPath
    if ($LASTEXITCODE -ne 0) {
      throw "Python repair helper failed with exit code $LASTEXITCODE."
    }
    Write-SeekDeepPass "Applied regenerate cooldown repair"
  ...
  Write-Host ""
  Write-Host "SeekDeep regenerate cooldown repair completed successfully." -ForegroundColor Green
