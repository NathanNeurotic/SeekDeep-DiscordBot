# SeekDeep / Seekotics archive @user mention route repair
#
# Fixes:
# - @SEEKOTICS archive shared works
# - @SEEKOTICS archive me works
# - @SEEKOTICS archive @user can fail because mention-cleaning removes the user mention
#   before the archive-open detector checks for it.
#
# Behavior after patch:
# - @SEEKOTICS archive @GLP-2 Roooo
# - @SEEKOTICS archive <@USERID>
# - @SEEKOTICS open archive @user
# all route locally and return the mentioned user's archive thread.
...
function Write-Info { param([string]$Message) Write-Host "[SeekDeep archive-mention-route] $Message" -ForegroundColor Cyan }
...
try {
  ...
  $backup = Join-Path $backupDir "index.js.before-archive-mention-route-$stamp.bak"
...
'@
  ...
  Write-Host ""
  Write-Pass "Archive @user mention route patch completed."
  ...
  Write-Host "@SEEKOTICS archive @GLP-2 Roooo" -ForegroundColor White
  exit 0
} catch {
  Write-Host ""
