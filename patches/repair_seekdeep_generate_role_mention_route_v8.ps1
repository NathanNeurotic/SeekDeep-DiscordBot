...
$IndexBackupPath = Join-Path $BackupsDir "index.js.before-generate-role-mention-route-v8-$Stamp.bak"
$PatchJsPath = Join-Path $PatchesDir "apply_generate_role_mention_route_v8_$Stamp.cjs"
...
try {
  ...
  Write-Host "- Leading bot user mentions and role mentions are normalized before image routing."
  Write-Host "- @SeekDeep generate a goomba should route as image instead of Qwen chat."
  Write-Host "- @SeekDeep draw me a goomba should keep working."
  Write-Host "- Image prompts should extract to 'a goomba' instead of '@SeekDeep generate a goomba'."
} catch {
  Write-Host ""
