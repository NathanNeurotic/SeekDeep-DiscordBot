# SeekDeep / Seekotics repair reply-context, raw pass-through, reply translation, and Discord explicit-media block
#
# Fixes current checkpoint issues:
#
# 1. Raw/unrefined options are parsed but not passed into normal message image generation:
#      makeImageResult(prompt, width, height, seed)
#    becomes:
#      makeImageResult(prompt, width, height, seed, seekdeepImageModeOptions)
#
# 2. Reply-context is too aggressive:
#      reply to "O kurwa..." with "@SEEKOTICS generate"
#    should NOT turn the vulgar reply text into a chat prompt.
#
# 3. Reply translation should use the replied-to message:
#      reply to Polish text with "@SEEKOTICS translate this to english"
#    should translate the replied text, not the literal phrase "translate this to english".
#
# 4. DiscordAPIError[20009] explicit media block should not crash/hang the queue.
#    If Discord blocks an image attachment, the bot posts a clean local notice instead.
#
# Files patched:
# - index.js
#
...
function Write-Info {
  ...
  Write-Host "[SeekDeep reply-raw-explicit-repair] $Message" -ForegroundColor Cyan
}
...
try {
  ...
  $indexBackup = Join-Path $backupDir "index.js.reply-raw-explicit-repair-$stamp.bak"
...
'@
  ...
  $patchPyPath = Join-Path $patchesDir "repair_reply_context_raw_explicit.py"
  [System.IO.File]::WriteAllText($patchPyPath, $patchPy, $utf8NoBom)

  Push-Location $projectRoot
  try {
    Write-Info "Applying reply/raw/explicit repair"
    & ".\.venv\Scripts\python.exe" $patchPyPath $indexPath
    if ($LASTEXITCODE -ne 0) {
      throw "Patch helper failed with exit code $LASTEXITCODE."
    }
    Write-Pass "Applied reply/raw/explicit repair"
  ...
  Write-Host ""
  Write-Pass "Reply/raw/explicit repair completed."
  ...
  Write-Host ""
  Write-Host "2) Reply to non-visual Polish/profanity text with: @SEEKOTICS generate" -ForegroundColor White
  Write-Host "   Expected: asks what to generate; does NOT send replied text to chat" -ForegroundColor White
  Write-Host ""
  Write-Host "3) Reply to that same Polish text with: @SEEKOTICS translate this to english" -ForegroundColor White
  Write-Host "   Expected: route=reply-translate and direct translation" -ForegroundColor White
  Write-Host ""
  Write-Host "4) If Discord blocks an image as explicit media, queue should finish with a clean text notice instead of error spam." -ForegroundColor White
  exit 0
} catch {
  Write-Host ""
