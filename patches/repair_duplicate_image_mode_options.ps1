# SeekDeep / Seekotics repair duplicate seekdeepImageModeOptions declaration
#
# Fixes:
#   SyntaxError: Identifier 'seekdeepImageModeOptions' has already been declared
#
# Cause:
# - The previous raw/reply patch partially applied and left two const seekdeepImageModeOptions
#   declarations inside the same image sender function scope.
#
# This repair:
# - Backs up index.js
# - Removes duplicate raw-mode option blocks inside:
#     seekdeepSendImageWithButtonsMessage(...)
#     seekdeepSendImageWithButtonsInteraction(...)
# - Ensures raw/unrefined options still pass to makeImageResult(...)
# - Ensures message-route raw tokens survive extraction
# - Ensures reply-context generate can force image routing
# - Runs syntax checks
...
function Write-Info {
  ...
  Write-Host "[SeekDeep duplicate-options-repair] $Message" -ForegroundColor Cyan
}
...
try {
  ...
  $indexBackup = Join-Path $backupDir "index.js.duplicate-image-mode-options-repair-$stamp.bak"
...
'@
  ...
  $patchPyPath = Join-Path $patchesDir "repair_duplicate_image_mode_options.py"
  [System.IO.File]::WriteAllText($patchPyPath, $patchPy, $utf8NoBom)

  Push-Location $projectRoot
  try {
    Write-Info "Repairing duplicate image mode options"
    & ".\.venv\Scripts\python.exe" $patchPyPath $indexPath
    if ($LASTEXITCODE -ne 0) {
      throw "Patch helper failed with exit code $LASTEXITCODE."
    }
    Write-Pass "Duplicate image mode options repair applied"
  ...
  Write-Host ""
  Write-Pass "Duplicate image mode options repair completed."
