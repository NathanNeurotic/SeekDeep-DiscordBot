# SeekDeep / Seekotics restore migrate-archive message command
#
# Fixes:
# - "@SEEKOTICS migrate archive" falling through to Qwen/chat instead of running locally.
#
# Adds/restores:
...
# - early messageCreate interception before chat routing
#
# Behavior:
# - "@SEEKOTICS migrate archive" imports old local images into the server archive channel's
#   "Shared" thread by default.
# - It imports up to 25 images per run.
# - It writes a ".discord-thread-migrated" marker beside each imported local image to avoid duplicates.
#
# Requirements:
# - Existing archive helper seekdeepGetOrCreateGuildArchiveChannel(...) from the thread archive patch.
# - If missing, the script adds a fallback version.
#
# Files changed:
# - index.js only
...
function Write-Info { param([string]$Message) Write-Host "[SeekDeep migrate-archive-route] $Message" -ForegroundColor Cyan }
...
try {
  ...
  $backup = Join-Path $backupDir "index.js.before-migrate-archive-route-$stamp.bak"
...
'@
  ...
  $patchPyPath = Join-Path $patchesDir "patch_migrate_archive_route.py"
  [System.IO.File]::WriteAllText($patchPyPath, $patchPy, [System.Text.UTF8Encoding]::new($false))

  Push-Location $projectRoot
  try {
    Write-Info "Applying migrate-archive message route patch"
    & ".\.venv\Scripts\python.exe" $patchPyPath $indexPath
    if ($LASTEXITCODE -ne 0) { throw "Patch helper failed with exit code $LASTEXITCODE." }
    Write-Pass "Applied migrate-archive message route patch"
  ...
  Write-Host ""
  Write-Pass "Migrate-archive message route patch completed."
  ...
  Write-Host "@SEEKOTICS migrate archive" -ForegroundColor White
  Write-Host ""
  Write-Host "Expected:" -ForegroundColor Cyan
  Write-Host "Archive migration pass complete." -ForegroundColor White
  Write-Host "Target thread: Shared" -ForegroundColor White
  exit 0
} catch {
  Write-Host ""
