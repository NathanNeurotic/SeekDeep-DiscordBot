# SeekDeep / Seekotics archive scope + path privacy repair v2
#
# Fixes the v1 patch failure:
#   Required anchor not found: dir helper
#
# Cause:
# - v1 tried to replace/add archive helpers conditionally and the final validation expected
#   seekdeepArchiveDirForTarget even when the insertion branch did not create it.
#
# v2 behavior:
# - Always ensures every archive helper exists, one by one.
# - Makes archive status/postarchive calls prefer current interaction/message guild scope.
# - Redacts host filesystem paths from Discord-facing archive output.
# - Does not delete or migrate existing files.
#
# Safety:
...
function Write-Info {
  ...
  Write-Host "[SeekDeep archive-scope-privacy-v2] $Message" -ForegroundColor Cyan
}
...
try {
  ...
  $indexBackup = Join-Path $backupDir "index.js.archive-scope-privacy-v2-$stamp.bak"
...
'@
  ...
  $patchPyPath = Join-Path $patchesDir "patch_archive_scope_privacy_v2.py"
  [System.IO.File]::WriteAllText($patchPyPath, $patchPy, $utf8NoBom)

  Push-Location $projectRoot
  try {
    Write-Info "Applying archive scope/privacy repair v2"
    & ".\.venv\Scripts\python.exe" $patchPyPath $indexPath
    if ($LASTEXITCODE -ne 0) {
      throw "Patch helper failed with exit code $LASTEXITCODE."
    }
    Write-Pass "Applied archive scope/privacy repair v2"
  ...
  Write-Host ""
  Write-Pass "Archive scope/privacy repair v2 completed."
  ...
  Write-Host "2) Archive a new image in that server." -ForegroundColor White
  ...
  Write-Host "   Expected: checks the server archive scope, not dm-unknown." -ForegroundColor White
  exit 0
} catch {
  Write-Host ""
