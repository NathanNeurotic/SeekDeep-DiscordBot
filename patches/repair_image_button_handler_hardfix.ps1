# SeekDeep image button hard-fix
# Replaces the corrupted image button handler with a clean implementation.
# Also preserves original/refined regenerate modes and stores image mode metadata
# in temp-image state so default regenerate follows the original submission mode.

$ErrorActionPreference = 'Stop'

function Write-Info { param([string]$Message) Write-Host "[SeekDeep image-button-hardfix] $Message" -ForegroundColor Cyan }
...
try {
  ...
  $backup = Join-Path $backupDir "index.js.image-button-hardfix-$stamp.bak"
...
'@
  ...
  Write-Host ''
  Write-Pass 'Image button hard-fix completed.'
  Write-Host "Backup created: $backup" -ForegroundColor Yellow
  Write-Host 'Restart the bot before retesting.' -ForegroundColor Yellow
  Write-Host ''
  Write-Host 'Retest:' -ForegroundColor Cyan
  Write-Host '1) Generate an image and click Original / Refined / Both.' -ForegroundColor White
  Write-Host '2) Click image result regenerate buttons.' -ForegroundColor White
  Write-Host '3) Confirm there is no customId or match ReferenceError in console.' -ForegroundColor White
  exit 0
}
catch {
  Write-Host ''
