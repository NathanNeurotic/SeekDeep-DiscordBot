# SeekDeep / Seekotics performance-snippet stabilizer patch
#
# Fixes issues shown in the performance snippet:
#
# 1) Image button failure:
#      Cannot access 'customId' before initialization
#    Cause:
#      the new regenerate parser references customId before the handler declares it.
#
# 2) Duplicate/stale parser residue after regenerate patch:
#    - stale match[] references
#    - duplicate const action / const actionId
#
# 3) Prompt-choice state lying:
#      "Queued refined prompt ... Both versions have now been queued."
#    when only one side was selected.
#    Cause:
#      previous row/helper logic can treat unrelated state fields as queued.
#
# 4) Regenerate should preserve mode:
#    - Original button => refinement off
#    - Refined button => refinement on
#    - Both => queue one original + one refined
#
# 5) Archive/status path privacy cleanup:
#    - hides local C:\ paths in Discord-facing archive/status output when passed through the redactor.
#
# Files patched:
# - index.js only
#
...
function Write-Info {
  ...
  Write-Host "[SeekDeep snippet-stabilizer] $Message" -ForegroundColor Cyan
}
...
try {
  ...
  $backup = Join-Path $backupDir "index.js.snippet-stabilizer-$stamp.bak"
...
'@
  ...
  $patchPyPath = Join-Path $patchesDir "patch_snippet_stabilizer.py"
  [System.IO.File]::WriteAllText($patchPyPath, $patchPy, $utf8NoBom)

  Push-Location $projectRoot
  try {
    Write-Info "Applying snippet stabilizer patch"
    & ".\.venv\Scripts\python.exe" $patchPyPath $indexPath
    if ($LASTEXITCODE -ne 0) {
      throw "Patch helper failed with exit code $LASTEXITCODE."
    }
    Write-Pass "Applied snippet stabilizer patch"
  ...
  Write-Host ""
  Write-Pass "Snippet stabilizer patch completed."
  ...
  Write-Host "1) Generate an image, then click image result buttons." -ForegroundColor White
  Write-Host "   Expected: no customId initialization error." -ForegroundColor White
  Write-Host "2) Click Original on a prompt chooser." -ForegroundColor White
  Write-Host "   Expected: only Original is queued; it should not claim both are done unless Refined was also queued." -ForegroundColor White
  Write-Host "3) Click Refined on an original result." -ForegroundColor White
  Write-Host "   Expected: refinement on only for Refined, not forced when Original was selected." -ForegroundColor White
  Write-Host "4) Run /archivestatus." -ForegroundColor White
  Write-Host "   Expected: no exposed C:\ path in Discord output if redaction path is used by that command." -ForegroundColor White
  exit 0
} catch {
  Write-Host ""
