# SeekDeep / Seekotics grounded image subject + generic image follow-up patch
#
# Purpose:
# - Route short visual subject phrases like:
#     "a bag of bells from Animal Crossing by Nintendo"
#   to image generation instead of chat.
#
# - When the user says a generic follow-up like:
#     "generate an image"
#     "draw it"
#     "show me that"
#   reuse the most recent image-subject prompt instead of literally generating:
#     "an image"
#
# - For named/franchise/product/game-item prompts, do a lightweight web-grounding pass
#   before image generation so the image model has visual facts instead of inventing blindly.
#
# Files patched:
# - index.js
#
...
# - UTF-8-safe patching
# - Does not touch image queue contract
# - Preserves:
...
function Write-Info {
  ...
  Write-Host "[SeekDeep grounded-image] $Message" -ForegroundColor Cyan
}
...
try {
  ...
  $indexBackup = Join-Path $backupDir "index.js.grounded-image-subject-context-$stamp.bak"
...
function seekdeepLooksLikeGroundableVisualSubject(prompt = '') {
  ...
  const namedCue = /\b(animal crossing|nintendo|pokemon|pok[eé]mon|zelda|hyrule|mario|sailor moon|sonic|playstation|ps2|xbox|minecraft|fortnite|roblox|disney|marvel|dc comics|star wars|final fantasy|pepe)\b/.test(p);
...
function seekdeepNeedsImageGrounding(prompt = '') {
  ...
  if (/\b(from|by|official|accurate|actual|game item|collectible|franchise)\b/.test(p) &&
      /\b(animal crossing|nintendo|pokemon|pok[eé]mon|zelda|hyrule|mario|sailor moon|sonic|playstation|ps2|xbox|minecraft|fortnite|roblox|disney|marvel|dc comics|star wars|final fantasy|pepe)\b/.test(p)) {
    return true;
  }

  if (/\b(bag of bells|bells bag|master sword|pok[eé]ball|mario mushroom|triforce|animal crossing)\b/.test(p)) return true;
...
'@
  ...
  $patchPyPath = Join-Path $patchesDir "patch_grounded_image_subject_context.py"
  [System.IO.File]::WriteAllText($patchPyPath, $patchPy, $utf8NoBom)

  Push-Location $projectRoot
  try {
    Write-Info "Applying grounded image subject/context patch"
    & ".\.venv\Scripts\python.exe" $patchPyPath $indexPath
    if ($LASTEXITCODE -ne 0) {
      throw "Patch helper failed with exit code $LASTEXITCODE."
    }
    Write-Pass "Applied grounded image subject/context patch"
  ...
  Write-Host ""
  Write-Pass "Grounded image subject/context patch completed."
  ...
  Write-Host "@SEEKOTICS a bag of bells from Animal Crossing by Nintendo" -ForegroundColor White
  Write-Host "@SEEKOTICS generate an image" -ForegroundColor White
  Write-Host ""
  Write-Host "Expected:" -ForegroundColor Cyan
  Write-Host "- first prompt routes to image" -ForegroundColor White
  Write-Host "- known/franchise subject is web-grounded before image generation" -ForegroundColor White
  Write-Host "- generic follow-up reuses the previous image subject instead of prompt='an image'" -ForegroundColor White
  exit 0
} catch {
  Write-Host ""
