...
$IndexBackupPath = Join-Path $BackupsDir "index.js.before-image-route-refine-v2-$Stamp.bak"
$LocalAiBackupPath = Join-Path $BackupsDir "local_ai_server.py.before-image-route-refine-v2-$Stamp.bak"
$PatchJsPath = Join-Path $PatchesDir "apply_image_route_refine_v2_$Stamp.cjs"
...
function insertBefore(src, marker, block, label) {
  ...
  if (firstLine && src.includes(firstLine)) return src;
...
// Make raw/unrefined image requests still carry a negativePrompt field.
if (!/negativePrompt:\s*['"]/.test(indexSource)) {
...
// Send negative_prompt to local_ai_server.py.
if (!/negative_prompt:\s*promptInfo\.negativePrompt/.test(indexSource)) {
  const seedLine = '    seed,\n';
  const postLocalIdx = indexSource.indexOf("  const response = await postLocal('/image', {");
  if (postLocalIdx < 0) throw new Error('Could not find makeImageResult postLocal /image block.');
  const seedIdx = indexSource.indexOf(seedLine, postLocalIdx);
  if (seedIdx < 0) throw new Error('Could not find seed line inside makeImageResult /image payload.');
  indexSource = indexSource.slice(0, seedIdx + seedLine.length) +
    "    negative_prompt: promptInfo.negativePrompt || process.env.SEEKDEEP_IMAGE_NEGATIVE_PROMPT || process.env.IMAGE_NEGATIVE_PROMPT || '',\n" +
    indexSource.slice(seedIdx + seedLine.length);
}
...
if (!indexSource.includes('// SEEKDEEP_DIRECT_IMAGE_ALIAS_MESSAGE_ROUTE_START')) {
  indexSource = insertBefore(indexSource, '    const utilityKind = seekdeepUtilityPromptKind(prompt);', directImageRouteBlock, 'direct image alias message route');
}
...
''
  ...
  pySource = pySource.replace(argsEndNeedle, argsEndNeedle + [
...
try {
  ...
  Write-Host "- Duplicate image button/job clicks are suppressed briefly."
} catch {
  Write-Host ""
