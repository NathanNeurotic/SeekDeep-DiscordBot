...
$IndexBackupPath = Join-Path $BackupsDir "index.js.before-shared-archive-button-ack-v3-$Stamp.bak"
$PatchJsPath = Join-Path $PatchesDir "apply_shared_archive_button_ack_v3_$Stamp.cjs"
...
function findClientInteractionCreateOpenBrace(src) {
  const patterns = [
...
const helperMarker = 'SEEKDEEP_SHARED_ARCHIVE_BUTTON_ACK_V3_START';
const helperBlock = String.raw`
// SEEKDEEP_SHARED_ARCHIVE_BUTTON_ACK_V3_START
function seekdeepSharedArchiveButtonLooksLikeSharedArchive(customId) {
...
async function seekdeepSharedArchiveButtonAckV3(interaction) {
  if (!interaction || !interaction.isButton || !interaction.isButton()) return false;
  if (!seekdeepSharedArchiveButtonLooksLikeSharedArchive(interaction.customId)) return false;
...
async function seekdeepSharedArchiveButtonRespondV3(interaction, content) {
...
function seekdeepSharedArchiveExtractPromptFromMessageV3(message) {
...
function seekdeepSharedArchiveCollectImageFilesV3(message) {
  const files = [];
  const seen = new Set();
...
async function seekdeepSharedArchiveButtonManualArchiveV3(interaction) {
  const message = interaction?.message || null;
  const guild = interaction?.guild || message?.guild || interaction?.channel?.guild || null;
  if (!guild) {
    await seekdeepSharedArchiveButtonRespondV3(interaction, 'Shared Archive only works inside a server.');
    return true;
  }
  ...
  if (typeof seekdeepGetOrCreateSharedArchiveThread === 'function') {
    ...
    sharedArchive = await seekdeepEnsureSharedArchiveThreadForChannel(archiveChannel, interaction, {
      source: 'shared-archive-button-ack-v3',
  ...
  const files = seekdeepSharedArchiveCollectImageFilesV3(message);
  if (!files.length) {
    await seekdeepSharedArchiveButtonRespondV3(interaction, 'Shared Archive could not find an image attachment on this Discord message. Nothing was archived.');
    return true;
  }

  const prompt = seekdeepSharedArchiveExtractPromptFromMessageV3(message);
  ...
  try {
    ...
    if (typeof seekdeepSharedArchiveSaveProfile === 'function') {
      seekdeepSharedArchiveSaveProfile(String(guild.id), {
        ...
        lastArchiveSource: 'shared-archive-button-ack-v3',
      });
    }
  } catch (countErr) {
  ...
  await seekdeepSharedArchiveButtonRespondV3(
    interaction,
...
async function seekdeepHandleSharedArchiveButtonInteractionV3(interaction) {
  if (!interaction || !interaction.isButton || !interaction.isButton()) return false;
  if (!seekdeepSharedArchiveButtonLooksLikeSharedArchive(interaction.customId)) return false;

  const start = Date.now();
  console.log('[SeekDeep] route=shared-archive-button-v3 customId=' + String(interaction.customId || ''));

  await seekdeepSharedArchiveButtonAckV3(interaction);

  try {
    await seekdeepSharedArchiveButtonManualArchiveV3(interaction);
  } catch (err) {
    ...
    console.log('[SeekDeep] shared-archive-button-v3 done in ' + (Date.now() - start) + 'ms');
  }

  return true;
}
// SEEKDEEP_SHARED_ARCHIVE_BUTTON_ACK_V3_END
`;

if (!out.includes(helperMarker)) {
  const hookIndex = findClientInteractionCreateOpenBrace(out);
  if (hookIndex < 0) {
    throw new Error('Could not find client.on("interactionCreate", ...) to install Shared Archive button ack hook.');
  }
  insertBeforeIndex(hookIndex, helperBlock + '\n\n');
  changes.push('inserted Shared Archive button ack/manual archive helper');
}

const hookLine = "  if (typeof seekdeepHandleSharedArchiveButtonInteractionV3 === 'function' && await seekdeepHandleSharedArchiveButtonInteractionV3(interaction)) return;";
if (!out.includes(hookLine)) {
  const openBrace = findClientInteractionCreateOpenBrace(out);
  if (openBrace < 0) throw new Error('Could not find interactionCreate handler opening brace for hook insertion.');
  ...
  changes.push('Shared Archive interactionCreate hook already present');
}
...
try {
  ...
  Write-Host "- Shared Archive button interactions are acknowledged immediately before archive work begins."
  Write-Host "- The button no longer depends on the older slow path that can time out and show 'This interaction failed'."
  Write-Host "- The button manually copies image attachments/embeds from the clicked result message into the shared archive thread."
  Write-Host "- Failures now return an ephemeral reason and log '[SeekDeep] shared archive button failed' instead of silently timing out."
} catch {
  Write-Host ""
