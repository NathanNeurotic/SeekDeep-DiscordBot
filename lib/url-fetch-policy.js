...
function seekdeepTranslatedV4FromIpv6(s) {
  ...
  // Strict /96 only: the NAT64 well-known prefix (RFC 6052) requires groups
  // 2-5 to be 0, and ONLY then is the embedded IPv4 the last 32 bits. Other
  // 64:ff9b:* addresses (e.g. the RFC 8215 local-use prefix 64:ff9b:1::/48)
  // embed the IPv4 at different positions — matching them here would extract
  // the wrong bytes (a false-block of a public addr, or worse, a false-pass of
  // a private one). Treat non-/96 64:ff9b:* as opaque public (no extraction).
  if (g[0] === 0x0064 && g[1] === 0xff9b && g[2] === 0 && g[3] === 0 && g[4] === 0 && g[5] === 0) {
    return { dotted: v4(g[6], g[7]), label: 'NAT64 64:ff9b::/96' };
  }
