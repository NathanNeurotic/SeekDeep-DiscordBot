...
export function seekdeepCompileReactionPattern(pattern = '') {
  ...
  // Apply \b only where the pattern edge IS a word char. A pattern that starts
  // or ends with a symbol/emoji (e.g. ":tada:", "🎉") has no word boundary there,
  // so an unconditional \b…\b would make it never match.
  const startB = /^\w/.test(raw) ? '\\b' : '';
  const endB = /\w$/.test(raw) ? '\\b' : '';
