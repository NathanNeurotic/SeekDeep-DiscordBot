...
**Accepted residual risk — Tauri IPC blast radius (audit M-1, 2026-06-18):** with `withGlobalTauri` on, the registered commands (`install_python_deps`, `install_ml_deps`, `install_torch_variant`, `retry_spawn`, `restart_sidecar`, `view_logs`, `try_start_docker_desktop`, `check_for_update`, `open_external`) are reachable by any script that executes in the WebView. The likelihood is low — `script-src 'self'` with zero inline scripts means arbitrary script execution requires a stored XSS that defeats the escaping renderers or a compromised bundled file. The **blast radius is bounded, not RCE**: every command's input is validated or fixed (the torch `variant` is allow-listed in `pip_install_torch_variant`; the rest are parameterless and act on fixed targets; `view_logs` opens only the fixed log dir; `open_external` is host-allowlisted), so a hostile script can only trigger *disruptive* actions (reinstall the known deps, restart the sidecar), not run arbitrary code. The clean fix (`withGlobalTauri:false` + per-command imports) requires moving the no-bundler vanilla-JS GUI to an ESM/bundled build — a tracked follow-up, not a hotfix. **Token-gating the commands is deliberately NOT done: `GET /token` is open, so an in-WebView script would just fetch the token first — it would be security theater.** Mitigation path when the GUI gains a build step: drop `withGlobalTauri`, or move the high-authority actions behind the token-gated Python backend.
