...
async function run() {
  ...
  if (!token || token.trim() === '') report('FAIL', 'DISCORD_TOKEN', 'Missing or empty in .env');
  else if (looksLikePlaceholder(token)) report('FAIL', 'DISCORD_TOKEN', 'Still set to a placeholder value');
  else report('PASS', 'DISCORD_TOKEN', 'Present (masked for security)');

  const clientId = process.env.DISCORD_CLIENT_ID;
  if (!clientId || clientId.trim() === '') {
    report('WARN', 'DISCORD_CLIENT_ID', 'Missing or empty. Message replies may still work, but slash/context command registration diagnostics are incomplete.');
  ...
  try {
    ...
    if (!['127.0.0.1', 'localhost'].includes(localAiUrl.hostname)) {
      report('WARN', 'LOCAL_AI_BASE_URL', 'Not loopback. Prompts/media may leave this machine if the server is remote.');
    }
  } catch (err) {
  ...
  const webSearchEnabled = envFlag(process.env.WEB_AUTO_SEARCH, true) || String(process.env.WEB_SEARCH_PROVIDER || '').toLowerCase() === 'searxng';
  ...
  if (webSearchEnabled) {
    try {
      ...
      if (res.ok || [401, 403, 404].includes(res.status)) report('PASS', 'SearXNG Gateway', `Reachable at ${searxngBaseUrlStr}`);
      else report('WARN', 'SearXNG Gateway', `Reachable but returned status ${res.status}`);
    } catch (err) {
      report('WARN', 'SearXNG Gateway', `Offline at ${searxngBaseUrlStr}: ${err.message}`);
    }
  } else {
    report('PASS', 'SearXNG Gateway', 'Disabled by WEB_AUTO_SEARCH/WEB_SEARCH_PROVIDER');
  }
  ...
  try {
    ...
    if (placeholderWarnings.length > 0) report('WARN', 'Secret Scanner', `Placeholder values still present: ${placeholderWarnings.join(', ')}`);
    else report('PASS', 'Secret Scanner', 'No obvious default placeholders remaining');
  } catch (err) {
