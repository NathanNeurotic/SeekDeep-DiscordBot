...
backup = Path(f"index.js.bak-temp-cache-helper-repair2-{stamp}")
...
start_marker = "// SEEKDEEP_TEMP_IMAGE_CACHE_START"
end_marker = "// SEEKDEEP_TEMP_IMAGE_CACHE_END"
...
if start != -1:
    ...
    if end == -1:
        raise SystemExit("Found temp cache start marker but not end marker.")

    end += len(end_marker)

    # eat trailing whitespace/newlines after old block
