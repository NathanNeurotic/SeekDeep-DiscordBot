const RELEASES = [
  { v: 'v10.38', kind: 'major', tagline: 'GUI⇄Discord parity · self-service TTS · CSP Phase A · DataDash Activity · audit round-2', tags: ['feat','fix','perf'], items: [
    '<strong>Onboarding review fixes — the blocking chat-model gate no longer hard-locks the default setup:</strong> three follow-ups from code review of the new onboarding gates. (1) <strong>Hard-lock fix (important).</strong> Making &ldquo;chat model installed&rdquo; a <em>blocking</em> first-run check exposed a latent bug: the cache probe read <span class="mono">HF_HOME</span> from the <span class="mono">.env</span> dict (usually blank) and fell back to <span class="mono">~/.cache/huggingface</span> — but this project caches models in <span class="mono">&lt;repo&gt;/models/huggingface</span> (via <span class="mono">LOCAL_MODEL_CACHE_DIR</span>), which the server force-sets into the <em>process</em> env at boot. So the default install read &ldquo;no chat model&rdquo; and the now-blocking gate would <em>trap first-run even with every model present</em>. The probe now checks the process env (<span class="mono">os.getenv</span> for <span class="mono">LOCAL_MODEL_CACHE_DIR</span> / <span class="mono">HF_HUB_CACHE</span> / <span class="mono">HF_HOME</span>) plus the repo-local default, and — to avoid the opposite error — confirms an actual <span class="mono">models--*</span> repo dir rather than &ldquo;directory is non-empty&rdquo; (the cache dir gets a <span class="mono">CACHEDIR.TAG</span>/<span class="mono">.locks</span> and the repo ships a <span class="mono">.gitkeep</span>, so a bare non-empty check would falsely pass on a model-less install and defeat the gate). (2) <strong>HF_TOKEN check now honours a globally-exported token</strong> (<span class="mono">os.getenv</span>, not just <span class="mono">.env</span>) so a token set in the shell/Docker/CI no longer shows a false &ldquo;not set&rdquo; warning. (3) <strong>Docker &ldquo;Install (winget)&rdquo; button is one-shot:</strong> on a successful install it stays <span class="mono">✓ Installed</span> and hides itself instead of resetting after 6s into a clickable state that could kick off a redundant multi-GB re-install. (Server/GUI — applies on the next server update.)',
    '<strong>Onboarding: one-click &ldquo;bring up web search&rdquo; (Docker&rarr;SearXNG, cold start):</strong> the System Check&rsquo;s SearXNG row only ever ran the container — which fails if the Docker daemon is down, so a cold first-run meant clicking &ldquo;Try start Docker&rdquo;, waiting 20-30s, <em>then</em> &ldquo;Start SearXNG&rdquo;. The desktop app&rsquo;s SearXNG button now runs a single atomic <strong>Tauri command</strong> (<span class="mono">start_searxng_stack</span>) that launches Docker Desktop if needed, <em>waits</em> for the daemon (up to ~2 min, with live progress), then starts the container. It deliberately lives in the desktop shell — the always-available layer — so it can recover web search even when the Python AI server is wedged, and it reuses the <em>same</em> container name / port (8080) / config volume (<span class="mono">&lt;runtime&gt;/searxng</span>) as the Python boot path so the two never diverge. The browser still falls back to the existing <span class="mono">POST /docker/start-searxng</span> (daemon-must-be-up) path. Closes the last zero-state onboarding gap. (Desktop-shell/Rust change — takes effect after the next installer rebuild, not via in-app self-update.)',
    '<strong>Onboarding: first run now hard-blocks until a chat model is installed, and prompts for an HF token:</strong> two more zero-state gaps closed. (1) <strong>No chat model is now a blocking first-run check.</strong> A fresh install with no <span class="mono">LOCAL_CHAT_MODEL_ID</span> (or one set but with no reachable HF cache / Ollama daemon) can&rsquo;t do its core job — chat, prompt-refine, routing — yet the &ldquo;Chat model installed and configured&rdquo; check was only a soft warning, so the wizard let you finish into a broken state. It&rsquo;s now <span class="mono">blocking</span>: first-run won&rsquo;t complete until a model is installed, and the one-click curated picker makes it resolvable in-app. (2) <strong>New <span class="mono">HF_TOKEN</span> first-run prompt.</strong> The default chat model (Llama-3.1) is <em>gated</em> on Hugging Face, so a tokenless fresh install silently couldn&rsquo;t download it. A new non-blocking check prompts for a Hugging Face access token (secret field, <span class="mono">hf_</span>-format validated, saved to <span class="mono">.env</span> via <span class="mono">/config</span>) — skip it and you fall back to the ungated Granite model, which works. (Server-side — applies on the next server update.)',
    '<strong>Installer: one-click Docker install on Windows:</strong> the System Check&rsquo;s Docker row only ever offered an external docker.com link. When &ldquo;Try start&rdquo; reports Docker isn&rsquo;t installed, Windows users now get an <strong>Install (winget)</strong> button that runs <span class="mono">winget install Docker.DockerDesktop</span> via the (already-present) <span class="mono">/system/install-docker</span> endpoint, with the docker.com link kept as the fallback (and on macOS/Linux). First onboarding-gap fix; the rest (first-run model download, Docker&rarr;SearXNG auto-chain, gated-default-model handling) are queued behind a couple of product decisions.',
    '<strong>/livez watchdog logs a failed kill:</strong> tiny follow-up — if the wedge-recovery watchdog&rsquo;s <span class="mono">child.kill()</span> ever fails (e.g. permissions), it now logs instead of swallowing the error, so a server that stays wedged despite the watchdog is diagnosable. (Usually the &ldquo;already exited&rdquo; benign case.) Desktop-shell/Rust — activates on the next installer rebuild. From a code review.',
    '<strong>Review hardening of the server-resilience fixes:</strong> four follow-ups from code review on the wedge/matcher work. (1) Bot-process detection now scans <em>every</em> command-line arg with strict <span class="mono">index.js</span> matching (exact / ends-with <span class="mono">/index.js</span> or <span class="mono">\\index.js</span>) — a stray <span class="mono">--require=x-index.js</span> can no longer mis-select and hide the real bot, and <span class="mono">some-index.js</span> no longer false-matches. (2) The uvicorn keep-alive/concurrency env knobs now fall back to defaults on a malformed value instead of crashing the server at startup. (3) <span class="mono">/health</span> runs its Ollama + GPU probes <em>concurrently</em> (max latency, not sum). (4) The new <span class="mono">/livez</span> watchdog kills the AI server via its own child handle rather than &ldquo;whatever holds the port&rdquo; — no port-race, no shelling out to taskkill/lsof.',
    '<strong>Watchdog now auto-recovers a wedged AI server (safety net):</strong> the existing crash-watchdog only respawns the server when its process <em>exits</em> — it can&rsquo;t see one that stays alive but stops serving. A new health-watchdog in the desktop shell probes the dependency-free <span class="mono">/livez</span> every 15s and, after ~75s of confirmed unresponsiveness while the process is still alive (and not mid-boot or mid-restart), force-kills it so the normal respawn path fires. The long threshold avoids false-kills during a legitimate model load. Pairs with the <span class="mono">/health</span> de-blocking fix: that removes the most common wedge, this catches anything left so you stop having to kill it by hand. (Desktop-shell/Rust change — takes effect after the next installer rebuild, not via in-app self-update.)',
    '<strong>Fixed the recurring &ldquo;server alive but not responding&rdquo; wedge (root cause):</strong> the AI server would sometimes sit running yet stop answering <span class="mono">/health</span> (every GUI pane goes UNKNOWN / the bot&rsquo;s circuit breaker trips) until it was killed by hand. Root cause: <span class="mono">/health</span> ran three <em>blocking</em> calls directly on the async event loop — <span class="mono">nvidia-smi</span> and two Ollama probes — so when the GPU was busy mid-image-gen or the Ollama socket hung, the whole loop blocked and health checks timed out even though the process was alive. (The <span class="mono">/gpu</span> endpoint already offloaded <span class="mono">nvidia-smi</span> to a worker thread; <span class="mono">/health</span> just never got the same treatment.) Now all three run via <span class="mono">to_thread</span>, plus a new dependency-free <span class="mono">/livez</span> liveness endpoint and bounded uvicorn keep-alive/concurrency. This removes the most common wedge; a watchdog safety-net for any remaining case ships alongside.',
    '<strong>Security: patched the <span class="mono">undici</span> HTTP advisory (via discord.js):</strong> a newly-published high-severity advisory flagged <span class="mono">undici 6.24.1</span> (HTTP header injection via Set-Cookie + response-queue poisoning + 2 more), pulled in transitively through <span class="mono">discord.js &rarr; @discordjs/rest</span>. Added an npm <span class="mono">overrides</span> forcing <span class="mono">undici ^6.27.0</span> (the patched 6.x — same major, API-compatible with @discordjs/rest 2.6.1), clearing all four. <span class="mono">npm audit</span> is back to 0 vulnerabilities and the bot loads clean (smoke 722/0). Avoided the <span class="mono">audit fix --force</span> path, which would have downgraded to discord.js@13 (breaking).',
    '<strong>Bot-process detection no longer reaps unrelated Node processes:</strong> the helper that finds the bot&rsquo;s <span class="mono">node index.js</span> (used by <span class="mono">/launcher/bot/kill-all</span>, the fresh-boot reaper, AND the periodic duplicate-bot watchdog) matched <em>any</em> node process running an <span class="mono">index.js</span> whose working directory was the SeekDeep folder. But processes the desktop app/host spawns — e.g. MCP servers running their own <span class="mono">&hellip;/index.js</span>, or Adobe&rsquo;s bundled node — <em>inherit</em> that working directory, so the watchdog kept silently killing them as &ldquo;duplicate bots&rdquo; (e.g. an MCP server flapping disconnected/reconnected). It now resolves the actual <span class="mono">index.js</span> path each process is running and only matches the bot&rsquo;s own <span class="mono">&lt;bot_cwd&gt;/index.js</span>. Surfaced live when a kill-all hit an unrelated MCP server.',
