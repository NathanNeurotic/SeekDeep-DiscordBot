const RELEASES = [
  { v: 'v10.38', kind: 'major', tagline: 'GUI⇄Discord parity · self-service TTS · CSP Phase A · DataDash Activity · audit round-2', tags: ['feat','fix','perf'], items: [
    '<strong>Docker probe pins the absolute CLI on macOS too:</strong> a follow-up to the L-4 PATH-hijack hardening (which only covered Windows). The Docker-start probe now prefers Docker Desktop&rsquo;s standard absolute binary on macOS (<span class="mono">/Applications/Docker.app/Contents/Resources/bin/docker</span>) so a polluted <span class="mono">PATH</span> can&rsquo;t shadow it, with the same bare-<span class="mono">docker</span> PATH fallback for non-standard installs (Homebrew, Colima, rootless). Linux/BSD stay on PATH resolution — CLI locations there vary by distro/runtime with no single canonical path to pin. From a code review of the L-4 PR.',
