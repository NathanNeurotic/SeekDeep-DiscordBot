const RELEASES = [
  { v: 'v10.38', kind: 'major', tagline: 'GUI⇄Discord parity · self-service TTS · CSP Phase A · DataDash Activity · audit round-2', tags: ['feat','fix','perf'], items: [
    '<strong>Code-review hardening (Gemini, round 4):</strong> the verb-group nav dropdowns now close each other when you open another (dropped an over-eager <span class="mono">stopPropagation</span>), and feature-gated items load under the macOS/Linux Tauri <span class="mono">tauri:</span> protocol (the gate only matched <span class="mono">http(s):</span> before — Windows was fine). The page registry is frozen read-only, the <span class="mono">.env</span> read-merge-write is serialized under a process lock, and the suppressed-alert toast fallback no longer prints "undefined" when an action has no detail body.',
